...
def predict(file_path):
    ...
    score = text_detection(image,H,W)
    return [H,W,score]
...
def resize_image(image_path, newW, newH,new_path):
    image = load_input_image(image_path)
    new_image = _resize(newW, newH, image)
    # name = image_path.split('/')[-1]
    # image_path[-1] = f'resized_{name}'
    # new_path = '/'.join(image_path)
    cv2.imwrite(new_path, new_image)

    return new_path 

def _resize(newW, newH, image):
